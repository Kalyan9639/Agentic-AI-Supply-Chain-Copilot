...
class VectorStore:
    ...
    def get_latest_assessments(self, limit: int = 10) -> List[RiskAssessment]:
        ...
        for metadata in results.get("metadatas", []) or []:
            if not metadata:
                continue
            assessments.append(self._assessment_from_metadata(metadata))

        assessments.sort(key=self._risk_sort_key)
