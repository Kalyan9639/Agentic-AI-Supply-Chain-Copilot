...
- Runtime config edits are stored in `data/config/runtime.env` and applied on the next backend reload or agent cycle.
...
- On Cloud Run, `data/config/runtime.env` is only durable if you back it with persistent storage or a secret/config service. For long-term production usage, Secret Manager or a database is a better fit than the local filesystem.
